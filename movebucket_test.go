...
func TestBucket_MoveBucket_DiffTx(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			err = db.View(func(tx *vmbolt.Tx) error {
...
func prepareBuckets(t testing.TB, tx *vmbolt.Tx, buckets ...string) *vmbolt.Bucket {
	var bk *vmbolt.Bucket
...
func openBucket(tx *vmbolt.Tx, bk *vmbolt.Bucket, bucketToOpen string) *vmbolt.Bucket {
	if bk == nil {
...
func createBucketAndPopulateData(t testing.TB, tx *vmbolt.Tx, bk *vmbolt.Bucket, bucketName string) *vmbolt.Bucket {
	if bk == nil {
...
func populateSampleDataInBucket(t testing.TB, bk *vmbolt.Bucket, n int) {
	var min, max = 1, 1024
